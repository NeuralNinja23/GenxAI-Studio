...
def test_marcus_governance_analyst_scoring(base_graph):
    ...
    res = MarcusGovernanceAnalyst.analyze_governance(unstable_branch)
...
def test_arbor_core_exploration_pipeline(base_graph, intent_field):
    core = ArborCore(max_branches=3)
    ...
    # Remaining 2 patches spawned branches and sorted
    assert len(active) == 3  # root + 2 new children
