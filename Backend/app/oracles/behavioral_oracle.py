    ...
    async def validate(self, project_id: str, project_path: Path, cycle_ctx: Any) -> OracleResult:
        ...
        for un in ui_nodes:
            ...
            has_state_binding = any(
                e.relation == "binds_state" and graph.nodes[e.target_id].node_type == NodeType.STATE_NODE
                for e in outgoing
            )
        ...
        state_nodes = [nid for nid, n in graph.nodes.items() if n.node_type == NodeType.STATE_NODE]
