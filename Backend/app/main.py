...
from app.sentinel.workflow import resume_workflow
...
async def lifespan(app: FastAPI):
    ...
    # Phase 6A: Initialize failure memory DB at startup.
    # Ensures failure_memory.db and its table exist immediately on boot,
    # not lazily on first failure — so the DB is always queryable.
    try:
        from app.sentinel.failure_memory.failure_recorder import FailureRecorder
        FailureRecorder.get_instance()
    except Exception as fm_err:
        log("Main", f"⚠️ FailureRecorder init error (non-fatal): {fm_err}")
