...
class TopologyBuilder:
    ...
    async def reconstruct(cls, project_id: str, project_path: Path) -> ProjectTopologyGraph:
        ...
        # Scan Backend
        backend_dir = WorkspaceArchitecture.backend_file(project_path, "app")
        if backend_dir.exists():
            cls._scan_backend(graph, backend_dir, project_path)

        # Scan Frontend
        frontend_dir = WorkspaceArchitecture.frontend_file(project_path, "src")
        if frontend_dir.exists():
            cls._scan_frontend(graph, frontend_dir, project_path)
    ...
    @classmethod
    def _scan_backend(cls, graph: ProjectTopologyGraph, backend_dir: Path, workspace: Path) -> None:
        ...
        if models_dir.exists():
            for p in models_dir.glob("**/*.py"):
                ...
                graph.add_node(
                    ...
                    properties={"file_path": WorkspaceArchitecture.to_workspace_relative(workspace, p), "entity_name": p.stem}
                )
        ...
        if api_dir.exists():
            for p in api_dir.glob("**/*.py"):
                ...
                graph.add_node(
                    ...
                    properties={"file_path": WorkspaceArchitecture.to_workspace_relative(workspace, p)}
                )
    ...
    @classmethod
    def _scan_frontend(cls, graph: ProjectTopologyGraph, frontend_dir: Path, workspace: Path) -> None:
