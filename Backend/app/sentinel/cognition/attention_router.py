...
class AttentionRouter:
    ...
    def calculate_branch_weight(
        ...
        $$\\text{Score} = (w_1(1.0 - \\text{Repulsion}) + w_2(\\text{Convergence}) + w_3(\\text{Simplicity})) \\times \\text{MarcusModifier}$$
        """
        w1 = 0.5
        w2 = 0.3
        w3 = 0.2

        # 1. Repulsion penalty (lower is better, meaning higher (1 - repulsion))
        repulsion_factor = 1.0 - min(branch.repulsion_score, 1.0)

        # 2. Convergence factor (closer to 0 change at final stages is better, but was evolving)
        ...
        # Apply Marcus advisory modifier (e.g. if Marcus warns of instability, modifier < 1.0)
        final_weight = float(max(raw_score * marcus_advisory_modifier, 0.01))

        # [VERIFY-1] Is repulsion_score always 0? Log every branch scoring to confirm.
